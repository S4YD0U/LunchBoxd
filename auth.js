...
const Auth = {
  ...
  /* AJOUTER UN AVIS */
  addReview: function(mealName, rating, comment) {
    var self = this;
    var session = this.getSession();
    if (!session) return Promise.resolve({ ok: false, error: 'Non connecté.' });
    var review = { id: Date.now().toString(), mealName: mealName, rating: rating, comment: comment, date: new Date().toISOString() };
  ...
  /* MODIFIER UN AVIS */
  editReview: function(reviewId, mealName, rating, comment) {
    var self = this;
    var session = this.getSession();
    if (!session) return Promise.resolve({ ok: false, error: 'Non connecté.' });
